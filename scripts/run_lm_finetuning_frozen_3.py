...
def train(args, train_dataset, model, tokenizer):
    ...
    # FREEZE THe Weights according to plan
    listing = []

    for x in model.named_parameters():
        listing.append(x)
    optimizer_grouped_parameters= (n for n in listing[2:38])   # 2 start + 12 per layer          10*12
    #
    
    optimizer_grouped_parameters = [
        {'params': [p for n, p in optimizer_grouped_parameters if not any(nd in n for nd in no_decay)], 'weight_decay': args.weight_decay},
        {'params': [p for n, p in optimizer_grouped_parameters if any(nd in n for nd in no_decay)], 'weight_decay': 0.0}
        ]
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    torch.save(checkpoint, os.path.join(output_dir, 'checkp.pth'))
...
def main():
    ...
    parser.add_argument('--load_states', type= bool  , default= False, help="whether to load past states")
    ...
    if args.load_states == True: 
        folder = [x[0] for x in os.walk(args.output_dir)][1]
        print(folder)
        states = torch.load(folder + "/pytorch_model.bin")
